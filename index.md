# 🌐 Azure Data Engineering Portfolio

[![Azure](https://img.shields.io/badge/Azure-Data%20Factory%20%7C%20Synapse%20%7C%20Databricks-blue)](https://azure.microsoft.com/)
[![Python](https://img.shields.io/badge/Python-ETL-green)](https://www.python.org/)
[![SQL](https://img.shields.io/badge/SQL-Synapse%20%7C%20SQL%20Server-orange)](https://learn.microsoft.com/en-us/sql/)
[![Power BI](https://img.shields.io/badge/BI-Power%20BI-purple)](https://powerbi.microsoft.com/)
[![CI/CD](https://img.shields.io/badge/CI%2FCD-GitHub%20Actions-black)](https://github.com/features/actions)

---

## 👋 About Me
I am a **Senior Data Engineer** with 9+ years of experience building **enterprise-scale, cloud-native data platforms** across banking, capital markets, telecom, and retail.

I specialize in:
- Cloud ETL / ELT pipelines  
- Data lakes & analytics platforms  
- Incremental & reconciliation processing  
- Executive-ready reporting & dashboards  

🔗 **Live Portfolio:**  
https://andiswamatai.github.io/azure-data-engineering-portfolio/

---

## 📌 Projects

### 1️⃣ Azure End-to-End Data Engineering Platform
Enterprise-grade **Azure data platform** ingesting on-prem SQL Server data into a governed analytics layer.

**Tech Stack**
- Azure Data Factory  
- ADLS Gen2  
- Databricks  
- Synapse Analytics  
- Power BI  
- Azure AD & Key Vault  

➡️ **View Case Study**  
[Azure End-to-End Data Engineering](projects/azure_data_engineering_case_study.md)

---

### 2️⃣ Azure Incremental / Streaming Pipeline
Incremental & near real-time pipeline showcasing **CI/CD**, event-driven ingestion, and analytics.

**Tech Stack**
- ADF Incremental Loads  
- Databricks Transformations  
- Event Hub / Kafka (conceptual)  
- Synapse Analytics  
- Power BI Streaming Dashboards  

➡️ **View Case Study**  
[Incremental & Streaming Pipeline](projects/azure_incremental_streaming.md)

---

## 📂 Documentation
- [Monitoring & Governance Overview](docs/Monitoring_Governance_Overview.docx)
- [Interview STAR Answers](docs/interview_star_answers.md)

---

## 📊 Visuals

### Power BI Dashboard
![Power BI Dashboard](images/powerbi_dashboard.png)

### Architecture & Pipelines
![ADF Pipeline](images/adf_pipeline.png)
![Databricks Transformation](images/databricks_transformation.png)
![Synapse Model](images/synapse_model.png)

---

## 🔗 Connect
- [GitHub](https://github.com/AndiswaMatai/azure-data-engineering-portfolio)
- [LinkedIn](https://www.linkedin.com/in/andiswamatai/)

